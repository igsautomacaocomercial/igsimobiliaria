...
async function openTransferComposition(id) {
  ...
  modal.innerHTML = `
    <div class="modal">
      ...
      <div class="table-wrap"><table><thead><tr><th>Contrato</th><th>Imovel</th><th>Competencia</th><th>Recebido</th><th>Aluguel</th><th>IPTU</th><th>Multa/Juros</th><th>Taxas</th><th>Liquido</th></tr></thead><tbody>${data.parcelas.map((p) => `<tr><td>${escapeHtml(p.contrato_codigo)}</td><td>${escapeHtml(p.imovel_titulo)}</td><td>${date(p.competencia)}</td><td>${money.format(Number(p.totais_repasse.recebido || 0))}</td><td>${money.format(Number(p.totais_repasse.aluguel || 0))}</td><td>${money.format(Number(p.totais_repasse.iptu || 0))}</td><td>${money.format(Number(p.totais_repasse.multa || 0) + Number(p.totais_repasse.juros || 0))}</td><td>${money.format(Number(p.totais_repasse.taxa_administracao || 0) + Number(p.totais_repasse.taxa_intermediacao || 0))}</td><td>${money.format(Number(p.totais_repasse.liquido || 0))}</td></tr>`).join('')}</tbody></table></div>
